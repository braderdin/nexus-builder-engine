...
const generateWhatsAppLink = (phoneNumber: string, message: string): string => {
  const cleanPhoneNumber = phoneNumber.startsWith("6") ? phoneNumber : `60${phoneNumber}`;
...
const WhatsAppQuickReplyModal: React.FC<WhatsAppModalProps> = ({ lead, onClose }) => {
  ...
  const messageTemplates: { [key: string]: string } = useMemo(() => ({
  ...
  const handleStatusUpdate = (leadId: string, newStatus: LeadStatus, openWhatsApp: boolean = false) => {
    ...
    if (openWhatsApp) {
      setLeads(currentLeads => {
        const updatedLead = currentLeads.find(lead => lead.id === leadId);
        if (updatedLead) {
                ...
                >
                  ...
                  {lead.status === 'new' && (
                    <div className="mt-4 pt-3 border-t border-slate-700 flex justify-center">
                      <button
                        onClick={() => handleStatusUpdate(lead.id, 'processing', true)}
              ...
              {leadsByStatus('processing').map((lead) => (
                ...
                >
                  ...
                  {lead.status === 'processing' && (
                    <div className="mt-4 pt-3 border-t border-slate-700 flex justify-center">
                      <button
                        onClick={() => handleStatusUpdate(lead.id, 'completed', true)}
