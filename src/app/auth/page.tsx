  ...
  const handleGoogleOAuthExecution = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          // Redirects user back to platform dashboard post-handshake successfully
    ...
    try {
      if (isLoginView) {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            data: { display_username: username }, // Injects profile metadata into auth stack
          },
        });
        if (error) throw error;
      ...
      // Maps Zod structured errors or database raw alerts down to display state safely
